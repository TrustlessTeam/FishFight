...
const UnityContent = createContext<UnityProviderContext | undefined>(undefined);
...
export const UnityProvider = ({ children }: UnityProviderProps) => {
  ...
  const [UnityInstance, setUnityInstance] = useState<UnityContext>(
    new UnityContext({
     loaderUrl: `${basePath}/Unity/fishfight-one-frontend.js`,
     dataUrl: `${basePath}/Unity/fishfight-one-frontend.data`,
     frameworkUrl: `${basePath}/Unity/fishfight-one-frontend.framework.js`,
     codeUrl: `${basePath}/Unity/fishfight-one-frontend.wasm`
    })
  );
  ...
  return (
    <UnityContent.Provider value={value}>{children}</UnityContent.Provider>
  );
};

// useFishFight
export const useUnity = () => {
  const context = useContext(UnityContent);
  if (!context) {
