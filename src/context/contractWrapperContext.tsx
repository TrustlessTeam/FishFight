		...
		setOnAccept(() => async () => {
			...
			.on('receipt', async (data: any) => {
				...
				toast.success('Breeding completed!', {
					onOpen: async () => {
						const fish = await createUserFish(Number(data.events.BreedingResult.returnValues.tokenId));
						if(fish != null) {
	...
	const contractDeathFight = (myFish: Fish, opponentFish: Fish, contractIsFighterDeposited: boolean) => {
		...
		setOnAccept(() => async () => {
			...
			.on('transactionHash', () => {
				...
				const fightResult = await getFightByIndex(Number(fightIndex), myFish)
