...
const FightingWatersWeak = () => {
	...
	// Context
	const { userFish, fightingFishWeak, loadingFish, loadingUserFish } = useFishPool()
	const { maxSupply, totalSupply, currentPhase, fightingWatersWeakSupply } = useFishFight()
			...
			<InfoContainer>
				...
				{!loadingFish && !loadingUserFish &&
					...
					{currentPhase?.phase === 2 ?
							<DataText>
								{`$FISHFOOD per Win: ${web3.utils.fromWei(Constants._fishFoodPerWinInPhaseWeak)} ->`} 
							...
							:
							<DataText>
								{`$FISHFOOD per Win: ${web3.utils.fromWei(Constants._fishFoodPerWinWeak)} ->`} 
								<StyledCountdown><Countdown renderer={renderer} date={new Date(timeTilPhase ? timeTilPhase * 1000 : 0)} /></StyledCountdown>
								{`then ${web3.utils.fromWei(Constants._fishFoodPerWinInPhaseWeak)}`} 
							</DataText>
						}
					</>
					...
					<DataText>
					{`DEPOSIT REWARD $FISHFOOD per Hour: ~${web3.utils.fromWei(Constants._fishFoodPerBlockWeakBN.mul(1800).div(fightingWatersWeakSupply+1).toString())}`}
					</DataText>
